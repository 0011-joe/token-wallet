...
## 6. 登录与发信（追加 · 对齐邮箱验证码 OTP）

> 本节为后续迭代追加，不改动上文历史条目；证据以当前代码/配置为准。

### 6.1 登录方式：邮箱验证码 OTP —— [已实现]
- 已从「魔法链接」切换为 **6 位数字验证码**：有效期 **10 分钟**，单码最多 **5 次**尝试，超限失效需重新获取。
- 准入仍由 `ALLOWED_EMAILS`（邮箱白名单）与 `INVITE_CODES`（邀请码）控制；两者都配置时叠加生效，未通过不发码。
- **[建议]** 公网部署两者至少配其一；仅依赖验证码而不设准入，等于向全网开放注册入口。

### 6.2 发件域名与 Resend 免费档限制 —— [建议]
- **必须验证自有域名**，`SMTP_FROM` 使用自有域名地址（如 `noreply@<your-domain>`）。
- `onboarding@resend.dev` 是 Resend 免费档未验证域名时的临时发件人，**通常只能发往 Resend 账号绑定邮箱**，不能作为生产发件人。
- 登录信与预警信共用同一发信通道；域名未验证时，两类邮件都可能被拒或只达本人。
- 详见 `docs/onboarding-email.md`。

### 6.3 `CRON_SECRET` 兼作 `/api/health` 鉴权 —— [已实现] / [建议]
- 快照端点与 `/api/health` 共用 `CRON_SECRET`（Bearer 或 `x-cron-secret` / `x-health-secret`）。
- **[建议]** 该密钥泄露可触发未授权快照与健康信息读取，按生产密钥同等保管（密码管理器 + 轮换流程）。

### 6.4 环境变量不入库 —— [已实现] / [建议]
- `.gitignore` 规则 `.env*` 覆盖 `.env` / `.env.local` / `.env.example` 等；**`.env.example` 不会出现在仓库中**，环境变量说明以 README 为准。
- **[建议]** 新人按 README 表格本地手写 `.env.local`，勿把含真实值的 env 文件误 `git add -f`。

### 6.5 Kimi / 豆包用量口径 —— [已实现]（产品语义）
- DeepSeek 用量为官方 CSV；Kimi / 豆包用量受接口限制，展示可能为估算，**不保证官方账单口径**，UI 已挂「估算」标识。
- 与安全无直接冲突，但涉及「数字可信度」披露义务，验收时须保留估算标注，不得呈现为官方精确账单。

### 6.6 快照 stale 提示 —— [已实现]
- 最近成功快照超过阈值（约 1.5 个快照周期）标 stale；仪表盘显著提示「数据陈旧 / 数据截至 xx:xx」，**不把失败显示成 0**（见上文 AC3.3）。
