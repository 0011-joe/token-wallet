...
export function renderMagicLinkEmail(data: MagicLinkEmailData): {
  ...
  const subject = "token-wallet 登录链接";
  ...
  <body style="margin:0;padding:24px;background:#f5f5f5;font-family:-apple-system,'Segoe UI',Roboto,'PingFang SC','Microsoft YaHei',sans-serif;">
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #e5e5e5;border-radius:12px;overflow:hidden;">
      <div style="padding:16px 24px;background:#0f766e;color:#ffffff;">
        <h2 style="margin:0;font-size:18px;font-weight:600;">登录 token-wallet</h2>
        <p style="margin:4px 0 0;font-size:12px;opacity:.9;">token-wallet · 魔法链接登录</p>
      </div>
      <div style="padding:24px;">
        <p style="margin:0 0 12px;font-size:14px;color:#333333;line-height:1.6;">您刚刚在 token-wallet 发起了登录请求，点击下方按钮即可完成登录：</p>
        <div style="margin:0 0 16px;text-align:center;">
          <a href="${href}" style="display:inline-block;padding:12px 32px;background:#0f766e;color:#ffffff;border-radius:8px;font-size:15px;text-decoration:none;">登录 token-wallet</a>
        </div>
        <p style="margin:0 0 12px;font-size:13px;color:#666666;line-height:1.6;">安全提示：此链接 24 小时内有效，点击后即失效；任何人获得此链接都可登录您的账户，请勿转发。</p>
        <hr style="margin:0 0 12px;border:none;border-top:1px solid #eeeeee;" />
        <p style="margin:0;font-size:12px;color:#999999;">如果这不是您本人操作，请忽略本邮件，您的账户仍然是安全的。本邮件由 token-wallet 自动发送，请勿直接回复。</p>
      </div>
    </div>
  </body>
</html>`;
