...
describeDb("upsertUsageImport 幂等覆盖（AC4-2）", () => {
  it("同月重复导入：importId 不变、行数不翻倍、元数据被覆盖", async () => {
    const parsed = parseUsageCsv(fs.readFileSync(AMOUNT_CSV, "utf8"));

    const first = await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_A, "first.csv", parsed.rows)
    );
    const second = await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_A, "second.csv", parsed.rows)
    );
    ...
    const imp = await db.usageImport.findUnique({
      where: { userId_provider_month: { userId: USER_ID, provider: "deepseek", month: MONTH_A } },
      include: { rows: true },
    });
    ...
    const totalCost = imp!.rows.reduce((s, r) => s + Number(r.cost.toString()), 0);
  ...
  it("不同月份互不影响", async () => {
    ...
    await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_B, "feb.csv", parsed.rows)
    );
...
describeDb("upsertUsageImport currency 语义（Q4：cost 文件币种）", () => {
  ...
  it("带 cost 文件导入 → currency 落库；再带不同币种 → 覆盖", async () => {
    ...
    await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_C, "c0.csv", parsed.rows)
    );
    let imp = await db.usageImport.findUniqueOrThrow({
      where: { userId_provider_month: { userId: USER_ID, provider: "deepseek", month: MONTH_C } },
    });
    expect(imp.currency).toBeNull();

    // 带 cost 文件（CNY）→ 落库
    await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_C, "c1.csv", parsed.rows, "CNY")
    );
    ...
    // 再带不同币种（USD）→ 覆盖
    await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_C, "c2.csv", parsed.rows, "USD")
    );
    imp = await db.usageImport.findUniqueOrThrow({
      where: { userId_provider_month: { userId: USER_ID, provider: "deepseek", month: MONTH_C } },
    });
  ...
  it("不带 cost 文件重复导入 → currency 保留旧值（不回退为 null）", async () => {
    const parsed = parseUsageCsv(fs.readFileSync(AMOUNT_CSV, "utf8"));
    await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_D, "d1.csv", parsed.rows, "CNY")
    );
    // 同月再只传 amount：update 分支不动 currency
    await db.$transaction((tx) =>
      upsertUsageImport(tx, USER_ID, "deepseek", MONTH_D, "d2.csv", parsed.rows)
    );
    const imp = await db.usageImport.findUniqueOrThrow({
      where: { userId_provider_month: { userId: USER_ID, provider: "deepseek", month: MONTH_D } },
    });
    expect(imp.currency).toBe("CNY");
